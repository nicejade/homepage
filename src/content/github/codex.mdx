---
title: Codex
description: OpenAI 官方终端编码 Agent，本地运行，支持 CLI、IDE 扩展与桌面应用。
...
Codex 是 OpenAI 推出的轻量级编码 Agent，可在本地终端运行，读取项目上下文并通过自然语言驱动读文件、改代码与执行命令。除 CLI 外，还提供 IDE 扩展（VS Code、Cursor、Windsurf）、桌面应用（`codex app`）与云端 [Codex Web](https://chatgpt.com/codex) 等多种入口。本仓库以 Apache-2.0 开源，承载 CLI 核心实现与贡献指南，适合希望把 OpenAI Agent 能力接入日常开发工作流的用户。

## 核心特性

- **终端原生 Agent**：在项目目录运行 `codex`，用对话完成跨文件编辑与命令执行
- **多入口形态**：CLI、IDE 扩展、桌面应用与 Codex Web 云端 Agent，覆盖不同工作习惯
- **ChatGPT 账户集成**：支持 Sign in with ChatGPT，纳入 Plus、Pro、Business、Edu 或 Enterprise 计划使用
...
- 相比 **OpenCode**：Codex 是 OpenAI 官方产品，账户与计费走 ChatGPT 计划；OpenCode 完全开源且模型无关，适合自由切换多家提供商。
...
也可从 [GitHub Releases](https://github.com/openai/codex/releases/latest) 下载对应平台二进制（如 `codex-aarch64-apple-darwin.tar.gz`）。若官方安装源不可用，可设置 `CODEX_INSTALLER_USE_RELEASES_OPENAI_COM=false` 强制走 GitHub Releases。
...
- 桌面体验可运行 `codex app` 或访问 [Codex App](https://chatgpt.com/codex?app-landing-page=true)；云端 Agent 请使用 [chatgpt.com/codex](https://chatgpt.com/codex)。
